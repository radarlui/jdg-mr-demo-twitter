...
function print_usage() {
	...
	echo "usage: $0 [--nodeCount[=]<value>]" >&2	
	echo "    --nodeCount=<value> is the node amount"
}
...
echo
	...
	setup_eap_with_modules
	echo "Starting EAP..."
	${JBOSS_HOME}/bin/standalone.sh -Djboss.node.name=master > /dev/null &
	sleep 10s
	echo "Deploying Webapp..."
	cd projects
	mvn clean package -pl JDG_MR_WEB jboss-as:deploy -pl JDG_MR_WEB -Djboss-as.hostname=127.0.0.1 -Djboss-as.port=9999 -Djboss-as.username=$USER -Djboss-as.password=$PASSWORD
	cd ..
	echo "Webapp running"
	for((i=1;i<=$NODECOUNT;i++))
	do
		echo "Setting up Node$i"
		setup_eap_node_with_modules "Node$i"
		echo "Starting Node$i..."
		./target/${NODE_NAME}/jboss-eap-6.4/bin/standalone.sh -Djboss.node.name=node$i -Djboss.socket.binding.port-offset=$((i*100))> /dev/null &
		sleep 10s
