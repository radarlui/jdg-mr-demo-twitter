...
import com.redhat.smelatam.services.TwitterDumper;
...
public class CacheConfig {
	...
	public org.infinispan.AdvancedCache<String,String> getLocalCache(){
		...
		if(localCache==null)
		{
			System.out.println("getLocalCache()");
		
			localCache = getLocalCacheManager().getCache("default");
		}
		TwitterDumper.reqCache = localCache.getAdvancedCache();
	...
	private EmbeddedCacheManager getLocalCacheManager() {
		...
		Configuration config = new ConfigurationBuilder()
			.clustering().cacheMode(CacheMode.DIST_ASYNC).hash().numOwners(1).expiration().maxIdle(3600, TimeUnit.SECONDS).build();
		if(dcm==null)
